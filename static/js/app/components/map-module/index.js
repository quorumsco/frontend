module.exports = {
  replace: false,
  template: require('../../../../Map/index.html'),
  attached: function() {
var debug = false;
...
$(function() {
  ...
  $("#select-data ul li a").on('click', function(e){
      e.preventDefault();
        ...
        if( ! subLayers_cache[n] ){
         ...
         displayInfos(currentRegLayer.feature, currentViewType);
                ...
                if (dataResultatsDirectory!="insee")
                {
                    ...
                    $('.communes').show().css("visibility", "visible").select();
                    $('.circonscriptions').show().css("visibility", "visible");
                    $('.iris').hide();
                    $('.pauvrete').hide();
                }else{
                    //$('#select-insee_subview').removeClass('hide');
                    $('.iris').show().css("visibility", "visible").select();
...
var dep_subview = 'communes'; // communes | circonscriptions
$(function() {
                ...
                if (dep_subview!="iris")
                        {
                                ...
                                if( ! subLayers_cache[n] )
                                {
                                        ...
                                        else if(dep_subview=='circonscriptions')
                                        {
                                                    var subLayers = new L.GeoJSON.AJAX(
                                        'data/contours/departements/'+currentDeptLayer.feature.properties.NUMERO+'/'+dep_subview+'.geojson',
                                            {
        ...
        if (dataResultatsDirectory!="insee")
                {
                    ...
                    $('.circonscriptions').show().css("visibility", "visible").select();
                }else{
                    //$('#select-insee_subview').removeClass('hide');
                    $('.iris').show().css("visibility", "visible").select();
                    $('.pauvrete').show().css("visibility", "visible");
...
function onEachFeatureReg(feature, layer) {
    ...
    if( dataResultatsDirectory != 'iris' ){
        $.ajax({
...
{ // sinon résultats de type INSEE
    ...
    //printDebug("dep_subview==pauvrete"+type,true);
        ...
        $.ajax({
                url: "data/resultats/"+dataResultatsDirectory+"/com/" + feature.properties.REF_INSEE.slice(0, 2) + "/" + feature.properties.REF_INSEE+ ".json",
                dataType: "json"
            })
            .done(function(data) {
